...
try:
    from flash_attn import DISABLE
...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
    ...
    if return_map:
        ## Sum all the heads' attention maps
        return attn_weight @ value, attn_weight.sum(dim = -3)
    else:
...
class RotaryEmbedding(nn.Module):
    ...
    def forward_from_seq_len(self, seq_len, target_len = None):
        device = self.inv_freq.device
        t = torch.arange(seq_len, device = device)

        if target_len != None:
            for i in range(seq_len):
                t[i] = int(i*target_len/seq_len)
        # print(t)
    ...
    def forward(
        ...
        if self.global_cond_dim is not None and self.global_cond_dim > 0 and global_cond is not None:
            ...
            x = self.self_attn(x, mask = mask, 
                               rotary_pos_emb = rotary_pos_emb, rotary_cond_emb = None)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.rotary_pos_emb is not None:
            rotary_pos_emb = self.rotary_pos_emb.forward_from_seq_len(seq_len=x.shape[1])  # (Tensor([seq_len, 32], 1)
            # print("1",rotary_pos_emb[0].shape, rotary_pos_emb[1])
        else:
            rotary_pos_emb = None
        if self.rotary_cond_emb is not None:
            rotary_cond_emb = self.rotary_cond_emb.forward_from_seq_len(seq_len=kwargs['context'].shape[1], target_len=x.shape[1])
